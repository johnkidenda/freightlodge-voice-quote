...
describe("app version 0.XX", () => {
  it("VERSION is 0.35 for this ship", () => {
    const raw = readFileSync("VERSION", "utf8").trim();
    expect(raw).toBe("0.35");
    expect(formatAppVersionLabel(raw)).toBe("v0.35");
    expect(formatAppVersionTitle("0.35", "abc1234")).toBe("v0.35 (abc1234)");
    expect(formatAppVersionTitle("0.35", "dev")).toBe("v0.35");
  });
