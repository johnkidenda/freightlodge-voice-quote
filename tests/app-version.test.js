...
describe("app version 0.XX", () => {
  it("VERSION is 0.23 for this ship (22 merged + this PR)", () => {
    const raw = readFileSync("VERSION", "utf8").trim();
    expect(raw).toBe("0.23");
    expect(formatAppVersionLabel(raw)).toBe("v0.23");
    expect(formatAppVersionTitle("0.23", "abc1234")).toBe("v0.23 (abc1234)");
    expect(formatAppVersionTitle("0.23", "dev")).toBe("v0.23");
  });

  it("bump script increments 0.XX by one", () => {
    expect(nextVersion("0.23")).toBe("0.24");
